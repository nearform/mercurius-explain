...
const wrapper = resolver => async (value, args, context, info) => {
  if (!context.explain.enabled) return resolver(value, args, context, info)

  context.explain.collector.addResolverCall(resolverCallPath(info))
  ...
  try {
    ...
    context.explain.collector.addEntry({
